...
function showSignature()
{
  ...
  function onGetSignature(sigCtlV, sigObjV, status)
  {
    if(wgssSignatureSDK.ResponseStatus.OK == status)
    {
      var outputFlags = wgssSignatureSDK.RBFlags.RenderOutputBase64 | wgssSignatureSDK.RBFlags.RenderColor24BPP;
  ...
  function onRenderBitmap(sigObjV, bmpObj, status) 
  {
    if(callbackStatusOK("Signature Render Bitmap", status))
    {
      ...
      //document.getElementById("imageBox").src="data:image/png;base64," + bmpObj;
      document.getElementById("patient-signature-image").value = "data:image/png;base64," + bmpObj;
