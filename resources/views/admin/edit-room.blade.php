							...
							<form class="form-horizontal" method="post" >
								...
								<div class="form-group {{ $errors->has('room-name') ? ' has-error' : '' }}">
									...
									<div class="col-sm-8">
										...
										<input type="text" class="form-control1" value="{{!empty($roomDetail)?$roomDetail->room_name:old('room-name')}}"   maxlength="30" name="room-name" id="room-name" placeholder="{{ __('menu.Room Name') }}">
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<div class="col-sm-8">
											<input value="{{!empty($roomDetail->duration)?$roomDetail->duration:''}}" id="duration" type="hidden" name="duration" placeholder="{{ __('menu.Duration') }}">
										</div>
									</div>
								</div>
								<div class="form-group {{ $errors->has('Color') ? ' has-error' : '' }}">
									...
									<div class="col-sm-8">
										...
										<input  type="text" class="form-control1" id="color-picker-component" value="{{!empty($roomDetail)?$roomDetail->room_color:old('Color')}}" readonly="" maxlength="30" name="Color" placeholder="{{ __('menu.Color Code') }}">
										...
										<select name="examination_type" class="form-control1">
											...
											<option value=" ">{{ __('menu.selectctexamination') }}</option>
												...
												@foreach($examination as $item)
													...
													<option value="{{$item->id}}" <?php echo($roomDetail->examination_type==$item->id)?'selected':''?>>{{$item->title}}</option>
